...
`@script`
Running a SQL command to order the results by highest to lowest sale price might seem simple enough at first, and running this query appears to work...but a closer look at the sale_price column reveals that this column is being stored as a string, NOT a number. These sale_price values are actually not perfectly ordered by sale_price.
